...
class EXP:
    def __init__(self, his_len=12, pre_len=12, lr=0.0001, batch_size=32, epochs=10, patience=3):
    ...
    def train(self):
        ...
        for e in range(self.epochs):
            self.model.train()
            train_loss = []
            for (batch_x, batch_y,batch_z) in tqdm(self.trainloader):
            ...
            with torch.no_grad():
                valid_loss = []
                for (batch_x, batch_y,batch_z) in tqdm(self.validloader):
                    pred, loss = self._process_one_batch(batch_x, batch_y)
                    valid_loss.append(loss.item())

                test_loss = []
                for (batch_x, batch_y,batch_z) in tqdm(self.testloader):
                    pred, loss = self._process_one_batch(batch_x, batch_y)
