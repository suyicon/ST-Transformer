...
class GraphMultiHeadAttention(nn.Module):
    def __init__(self, d_k, d_v, d_model, n_heads, dropout):
        ...
        self.W_Q = GCNLayer(self.d_model, self.d_k * self.n_heads)
        self.W_K = GCNLayer(self.d_model, self.d_k * self.n_heads)
        self.W_V = GCNLayer(self.d_model, self.d_v * self.n_heads)
        self.fc = nn.Linear(self.n_heads * self.d_v, self.d_model, bias=False)

        self.norm = nn.LayerNorm(self.d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, input_Q, input_K, input_V, adj):
        residual = input_Q.clone()
        Q = self.W_Q(input_Q,adj).view(input_Q.size(0), input_Q.size(1), input_Q.size(2), self.n_heads, self.d_k)
        K = self.W_K(input_K,adj).view(input_Q.size(0), input_Q.size(1), input_Q.size(2), self.n_heads, self.d_k)
        V = self.W_V(input_V,adj).view(input_Q.size(0), input_Q.size(1), input_Q.size(2), self.n_heads, self.d_v)
