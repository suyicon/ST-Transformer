...
class STBlock(nn.Module):
    def __init__(self, d_k, d_v, d_model, len_his, n_heads, n_nodes, dropout, adj):
        ...
        self.TTN = TemporalTransformer(d_k, d_v, d_model, len_his, n_heads, dropout,adj)
...
class STTN(nn.Module):
    def __init__(self, d_k=32, d_v=32, d_feature=1, d_model=64, len_his=12, len_pred=12, n_heads=8, n_nodes=25,
        ...
        self.encoder = Encoder(input_dim=d_feature,emb_dim=d_model,hid_dim=d_model,len_his=len_his,n_layers=1,dropout=0.5)
        self.decoder = Decoder(output_dim=d_feature,emb_dim=d_model,hid_dim=d_model,n_layers=1,dropout=0.5)
